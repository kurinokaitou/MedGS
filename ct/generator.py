...
def config_parser():
    ...
    parser.add_argument("--ctName", default="jaw", type=str,
                        help="Name of CT")
    parser.add_argument("--outputName", default="data", type=str,
                        help="Name of output data")
    parser.add_argument("--dataFolder", default="./data", type=str,
                        help="folder of raw data")
    parser.add_argument("--outputFolder", default="./dataset", type=str,
...
def main():
    ...
    matPath = f"./{dataFolder}/{dataType}/img.mat"
    configPath = f"./{dataFolder}/{dataType}/config.yml"
    outputPath = osp.join(outputFolder, f"{outputName}.pickle")
    generator(matPath, configPath, outputPath, outputFolder, True)
...
def generator(matPath, configPath, outputPath, outputFolder, show=False):
    """
    ...
    # lids = tigre.Ax(img.copy(), geo, np.array([0, np.pi]))[:, ::-1, :]
    if data["noise"] != 0 and data["normalize"]:
        ...
        # noise_lids = CTnoise.add(lids, Poisson=1e5, Gaussian=np.array([0, data["noise"]]))
        data["train"]["projections"] = noise_projections
        # data["train"]["lids"] = noise_lids
    else:
        data["train"]["projections"] = projections
        # data["train"]["lids"] = lids

    # Generate testidation images
    data["test"] = {"angles": np.sort(np.random.rand(data["numTest"]) * 180 / 180 * np.pi) + data["startAngle"]/ 180 * np.pi}
    projections = tigre.Ax(np.transpose(img, (2, 1, 0)).copy(), geo, data["test"]["angles"])[:, ::-1, :]
    if data["noise"] != 0 and data["normalize"]:
        print("Add noise to projections")
        noise_projections = CTnoise.add(projections, Poisson=1e5, Gaussian=np.array([0, data["noise"]]))
        # noise_lids = CTnoise.add(lids, Poisson=1e5, Gaussian=np.array([0, data["noise"]]))
        data["test"]["projections"] = noise_projections
        # data["test"]["lids"] = noise_lids
    else:
        data["test"]["projections"] = projections
        # data["test"]["lids"] = lids
    ...
    if not os.path.exists(f"{outputFolder}/images"):
        os.makedirs(f"{outputFolder}/images")
    for i, proj in enumerate(data["train"]["projections"]):
        proj_uint8 = (cv2.normalize(proj, None, 0, 1, cv2.NORM_MINMAX)  * 255).astype(np.uint8)
        proj_img = np.repeat(np.expand_dims(proj_uint8, axis=2), 3, axis=2)
        Image.fromarray(proj_img).save(f"{outputFolder}/images/projection{i}.png")
    # for i, proj in enumerate(lids):
    #     proj_uint8 = (cv2.normalize(proj, None, 0, 1, cv2.NORM_MINMAX)  * 255).astype(np.uint8)
    #     proj_img = np.repeat(np.expand_dims(proj_uint8, axis=2), 3, axis=2)
    #     Image.fromarray(proj_img).save(f"{outputFolder}/images/lids{i}.png")
